...
const renderTracks = (track, i) => {
   trackList.innerHTML += `<div class="songContainer" song-id="${track.id}" onclick="setPlayer(event)">
...
const renderArtists = (artist) => {
   artistList.innerHTML += `
   <div class="img-artist-container" onclick="location.href='artistpage.html?artistId=${artist.id}';">
...
const renderAlbums = (album, artist) => {
   albumsList.innerHTML += `<div class="card col p-3 p-0 playing-card" style="width: 11rem">
