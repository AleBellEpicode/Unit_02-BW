...
const retrieveBySong = async (songid) => {
   try {
      let res = await fetch(`https://striveschool-api.herokuapp.com/api/deezer/track/${songid}`)
      ...
      let artistPlayer = document.querySelector('#songInfoFooter > p:nth-child(2)')
...
const setPlayer = async (event) => {
   if (event.target.attributes['song-id']) {
      ...
      try {
         let resalbum = await fetch(`https://striveschool-api.herokuapp.com/api/deezer/album/${albumid}`)
